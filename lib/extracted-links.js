...
const URLFinalObject = (absPath, strNameAndURL) => {
  ...
  return { name, url, absPath };  //object con dos properties
};

const getAbsolutePath = (thePath) => {
  return (path.isAbsolute(thePath)) ? thePath : path.resolve(thePath); 
};
